...
import { QuestingSystemConfig, DEFAULT_QUESTING_SYSTEM_CONFIG, NeuralModelsConfig, DEFAULT_NEURAL_MODELS_CONFIG } from "../types";
...
export const QuestingSystemSettingsSection: React.FC = () => {
  ...
  const [activeQuestModel, setActiveQuestModel] = useState<string>("");
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/questing-system/config").then((res) => res.json()),
      fetch("/api/neural-models-config").then((res) => res.json())
    ])
      .then(([questData, neuralData]) => {
        setConfig(questData || DEFAULT_QUESTING_SYSTEM_CONFIG);
        if (neuralData?.questEngineModel) setActiveQuestModel(neuralData.questEngineModel);
        setLoading(false);
      })
      .catch((err) => {
  ...
  const handleSave = async () => {
    ...
    setSaveToast(null);
    try {
      const resQuest = await fetch("/api/questing-system/config", {
        method: "POST",
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-[#121216] border border-[#2A2A2E] p-4 rounded-xl space-y-2 relative overflow-hidden group hover:border-amber-500/30 transition-colors">
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-semibold text-gray-200">Quest Engine Model</label>
              <span className="text-xs font-mono text-amber-300 bg-amber-500/10 border border-amber-500/25 px-2 py-0.5 rounded">
                {activeQuestModel || "Configured Centrally"}
              </span>
            </div>
            <p className="text-xs text-gray-400 leading-relaxed mt-2">
              Model assignment for quest generation and objective evaluations is centrally managed in the <strong>Neural Model & Inference Servers</strong> tab.
            </p>
          </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-[#121216] border border-[#2A2A2E] p-4 rounded-xl space-y-2 relative overflow-hidden group hover:border-amber-500/30 transition-colors">
            <div className="flex items-center justify-between">
              <label className="text-sm font-semibold text-gray-200">Enable Automated Quest Narrative Injections</label>
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={config.enableAutomatedQuestNarrativeInjections !== false}
                  onChange={(e) => setConfig((prev) => ({ ...prev, enableAutomatedQuestNarrativeInjections: e.target.checked }))}
                  className="sr-only peer"
                />
                <div className="w-9 h-5 bg-[#2A2A2E] peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-gray-400 peer-checked:after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:bg-amber-500"></div>
              </label>
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              When enabled, milestone and completion triggers from Stage 3 are queued into the chat context for immediate narrative follow-up.
            </p>
          </div>
